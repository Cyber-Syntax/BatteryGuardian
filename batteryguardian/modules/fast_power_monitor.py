...
def setup_fast_ac_monitoring(callback: Callable[[int, str], None], state: Dict[str, Any]) -> bool:
    """
            ...
            try:
                ...
                def get_complete_status():
                    try:
                        ...
                        logger.debug("Background status update complete: battery=%d%%, AC=%s", 
                                    battery_percent, ac_status)
                ...
                def ac_signal_handler(*_args, **_kwargs):
                    try:
                        ...
                        device = bus.get_object("org.freedesktop.UPower", ac_adapter_path)
                        device_interface = dbus.Interface(device, "org.freedesktop.DBus.Properties")
                        ...
                        logger.info("Ultra-fast AC status update: %s (%.2f ms)", ac_status, elapsed_ms)
            ...
            try:
                ...
                while True:
                    try:
                        ...
                        device_interface = dbus.Interface(device, "org.freedesktop.DBus.Properties")
                        online = bool(device_interface.Get("org.freedesktop.UPower.Device", "Online"))
                        
                        # If status changed or this is the first check
                        if last_status["ac_online"] is None or last_status["ac_online"] != online:
                            start_time = time.time()
                            ...
                            logger.info("Direct polling detected AC status change: %s", ac_status)
